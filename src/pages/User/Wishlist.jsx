...
function Wishlist() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const requests = wishlist.map(async (item) => {
              ...
              <div className='flex flex-col xl:flex-row gap-5 xl:gap-0 items-center justify-between w-full'>
                <div className='flex flex-wrap md:flex-nowrap justify-center gap-5 items-start'>
                  ...
                  <div className='flex items-center sm:items-start flex-col gap-2'>
                    <span className='text-xl font-semibold flex-wrap justify-center flex items-end sm:gap-3'>{val?.title} <span className='text-gray-500'>{val?.category === "pcGames" ? <span>(PC)</span> : val?.category === "ps4Games" ? <span>(PS4)</span> : val?.category === "ps5Games" ? <span>(PS5)</span> : val?.category === "xboxGames" && <span>(XBOX)</span>}</span></span>
