...
function AdminOrders() {
  ...
  const fetchCount = async () => {
    ...
    try {
      ...
      const [all, processing, completed] = await Promise.all([
        supabase.from("orders").select("*"),
        supabase.from("orders").select("*").eq("orderStatus", "Processing"),
        supabase.from("orders").select("*").eq("orderStatus", "Completed")
      ])
