  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [
        ...
        { data: gamesData, count, error: gameError }
      ] = await Promise.all([
        supabase
          .from("requests")
          .select("*")
              ...
              {activePanel === "Notifications"
                          ...
                          <div className='overflow-hidden'>
                            ...
                            <div className='flex items-center justify-between'>
                              ...
                              <div className='flex items-center gap-1 mt-1'>
                                ...
                                <button onClick={(e) => { requestUpdate(val.id, "starredStatus", val.starredStatus === "Stared" ? "Unstared" : "Stared"), e.stopPropagation() }} className={`p-1.5 rounded-full bg-yellow-500/30 text-yellow-500 cursor-pointer hover:bg-yellow-500 hover:text-white`}>{val?.starredStatus === "Stared" ? <FaStar /> : <FaRegStar />}</button>
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                : activePanel === "Accepted" ?
                  ...
                  : activePanel === "Rejected" ?
                    ...
                    : activePanel === "Stared" ?
                      <div>
                        {requests?.filter((item) => item.starredStatus === "Stared")?.map((val, index) =>
                          <>
                            <div key={index} onClick={() => setRequestDetail(val)} className='p-3 px-5 flex items-start gap-3 border-b border-r border-gray-400 dark:border-[#011743] dark:hover:bg-[#011743] hover:bg-gray-200'>
                              <div className={`border mt-2 w-10 h-10 text-xl rounded-full flex items-center justify-center ${avatarColors[val?.firstName.at(0)]}`}><span>{val?.firstName.at(0)}</span></div>
                              <div className='w-[90%]'>
